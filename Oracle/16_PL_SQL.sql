/*
    ...
    - SQL 문장 내에서 변수의 정의, 조건처리(IF), 반복처리(LOPP, FOR, WHILE)등을 지원하여 SQL의 단점을 보완
...
-- DEPARTMENT 테이블의 DEPT_TITLE 컬럼의 자료형을 참조 사용자가 입력한 사번과 일치하는
-- 사원을 조회한 후 조회 결과를 출력
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
...
-- 사용자가 입력한 사원의 사번(EID), 이름(ENAME), 부서명(DTITLE), 근무국가코드(NCODE)
-- 조회 후 각 변수에 대입 일반타입변수 TEAM을 데이터 타입 VARCHAR2(10)으로 선언하고
-- NCODE 값이 KO일 경우 => TAEM에 '국내팀' 대입,
--                그게아닐경우 => TEAM에 '해외팀' 대입
...
-- D1인 경우 인사관리부, D2인 경우 회계관리부, D3인 경우 마케팅부, D4인경우 국내영업부,
-- D5인 경우  해외영업1부, D6인 경우 해외영업2부, D7인 경우 해외영업3부,
-- D8인 경우 기술지원부, D9인 경우 총무부 나머지는 부서없음
...
-- NO_DATA_FOUND : SELECT한 결과가 한 해도 없는 경우
-- TOO_MANY_ROWS : SELECT한 결과가 여러 행인 경우 (한 행이 리턴되어야 하는데 여러 행일 경우)
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
