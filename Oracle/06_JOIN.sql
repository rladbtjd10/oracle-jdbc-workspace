 /*
      ...
      - 관계형 데이터베이스는 최소한의 데이터로 각각의 테이블에 담고 있음
        ...
        부서 데이터는 부서 테이블, 사원에 대한 테이터는 사원 테이블, 직급 테이블....
 ...
 */
 ...
 /*
      ...
      - 연결 시키는 컬럼의 값이 일치하는 행들만 조인되서 조회 (일치하는 값이 없는 행은 조회X)
      ...
      1) 오라클 전용 구문
       ...
       2) ANSI 표준 구문
           ...
           - JOIN 절에서 같이 조회하고자 하는 테이블을 기술 후 매칭 시킬 컬럼에 대한 조건을 기술 (USING 구문과 ON 구문사용)
...
-- 해결방법1) 테이블명 또는 별칭을 이용해서 하는 방법
 ...
 -- 2. DEPARTMENT와 LOCATION을 참고해서 전체 부서의 부서코드, 부서명, 지역코드, 지역명을 조회
 --> 오라클 구문
 ...
 */
  ...
  FROM EMPLOYEE
  CROSS JOIN DEPARTMENT; -- 사원명23 X 부서9 = 207개나옴
...
--2. 70년대 이면서 여자이고, 성이 전 씨인 직원들의 직원명, 주민번호, 부서명, 직급명을 조회
--> ANSI 구문
