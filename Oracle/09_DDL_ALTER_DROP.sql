 ...
 */
 -- DEPT_ID 컬럼의 데이터 타입을 CHAR(3)으로 변경
  ...
  ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER; --오류 발생! (값이 있어서) --ORA-01439: 데이터 유형을 변경할 열은 비어 있어야 합니다
  ...
  -- LOCATION_ID 컬럼의 데이터 타입을 VARCHAR2(2), LNAME 컬럼의 기본값을 미국으로 변경
  ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE  VARCHAR2(40)
  MODIFY LOCATION_ID VARCHAR2(2) 
  MODIFY LNAME DEFAULT '미국'; --지정후에 테이블을 추가 한것들부터 미국으로 변경됨
...
-- DEPT_TEST 테이블 삭제
 ...
 -- 방법 2. 그냥 부모테이블만 삭제하는데 제약조건까지 같이 삭제하는 방법--권유하지않음
 DROP TABLE MEMBER_GRADE CASCDE CONSTRAINT;
