 ...
 */
 ...
 /*
     ...
     - 산술연산을 하게 되면 컬럼명이 지저분해진다.
       이때 컬럼명의 별칭을 부여해서 깔끔하게 보여줄 수 있다.
 ...
 */
 ...
 -- XXX(사원명)의 월급은 XXX(급여)원 입니다. "급여 정보"-별칭
 ...
 */
 ...
 -- WHERE DEPT_CODE <> 'D1';
  ...
  -- OR (또는), AND (그리고)
  -- 1. 부서코드가 'D6'이거나 급여가 300만원 이상인 사원들의 사원명, 부서코드, 급여조회
  SELECT EMP_NAME, DEPT_CODE, SALARY
  FROM EMPLOYEE
  WHERE DEPT_CODE = 'D6' OR SALARY >= 3000000; --숫자여서 자바처럼 ' ' 안해도 됨
...
-- 2. 급여가 350만원 이상 600만원 이하를 받는 사원들의 사원명, 사번, 급여 조회
 ...
 /*
     ...
     '_' : 1글자
     비교대상컬럼 LIKE '_문자' => 비교대상컬럼값이 문자앞에 무조건 한글자가 올 경우 조회
 ...
 */
 ...
 -- EX) sim_bs@kh.or.ke, sun_di@kh.or/kr 
 ...
 --WHERE NOT EMAIL LIKE '___$_%' ESCAPE '$' ; --로해도됨
 ...
 */
 ...
 FROM EMPLOYEE
 WHERE BONUS IS NULL; --보너스를 받으면 IS NOT NULL
 ...
 */
 ...
 --WHERE NOT DEPT_CODE  IN ('D6', 'D8', 'D5'); --이것도 될수 있음
 ...
 */
 ...
 /*
     ...
     - NULLS FIRST : 정렬하고자하는컬럼값에 NULL이 있을 경우 해당 데이터를 맨 앞 배치 (생략시 DESC일때 기본값)
     - NULLS LAST :  정렬하고자하는컬럼값에 NULL이 있을 경우 해당 데이터를 맨 뒤 배치 (생략시 ASC일때 기본값)
 */
 ...
 --WHERE BONUS IS NOT NULL; --NULL을 없애고 싶을때
