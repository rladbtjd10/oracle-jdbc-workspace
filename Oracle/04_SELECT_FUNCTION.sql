 /*
     ...
     - 단일행 함수 : N개의 값을 읽어서 N개의 결과값을 리턴(매 행마다 함수 실행 결과를 반환)
 ...
 */
 ...
 /*
     INSTR
     - INSTR(STRING, STR [, POSITIION, OCCURRENCE])
 ...
 */
 ...
 /*
     ...
     - 문자열에 덧붙이고자하는문자를 왼쪽 또는 오른쪽에 덧붙여서 최종 N길이만큼의 문자열을 반환
 */
 ...
 /*
     ...
     - 문자열의 왼쪽 또는 오른쪽에서 제거 하고자하는문자들을 찾아서 제거한 나머지 문자열을 반환
 */
 ...
 */
 ...
 SELECT TRIM( LEADING 'Z' FROM 'ZZZKHZZZ') FROM DUAL; -- LTRIM이랑 같음
 SELECT TRIM( TRAILING 'Z' FROM 'ZZZKHZZZ') FROM DUAL; -- RTRIM이랑 같은
 ...
 */
 ...
 SELECT CONCAT(CONCAT('가나다라', 'ABCD'), '1234') FROM DUAL; --2개의문자받게안되서 CONCAT을 한번더쓴것
 ...
 */
 ...
 -- 사원명, 이메일, 이메일의 kh.ro.kr를 gmail.com으로 변경해서 조회
 ...
 */
 ...
 -- 날짜 포맷 변경
 ALTER SESSION SET NLS_DATE_FORMAT = 'YYYY-MM-DD HH:MI:SS'; --먼저실행하고 날짜가 있는 함수를 실행시 변경된걸 확인할수있음
 ...
 */
 SELECT TO_CHAR(1234, 'L999999') FROM DUAL; --￦1234 현재 설정된 나라의 화폐단위
 ...
 SELECT EMP_NAME "직원명", TO_CHAR(SALARY, 'L999,999,999') "급여", TO_CHAR(SALARY*12, 'L999,999,999') as "연봉"
 FROM EMPLOYEE
 ORDER BY "연봉";
 ...
 */
 ...
 /*
      선택 함수 : 여러가지 경우에 선택을 할 수 잇는 기능을 제공하는 함수
 ...
 */
 ...
 -- 그외의 직급의 사원은 급여를 5%만 인상
 -- 정렬 : 직급 코드 J1 부터, 인상된 급여 높은 순서대로
 SELECT EMP_NAME , JOB_CODE, SALARY, DECODE(JOB_CODE, 'J7', SALARY * 1.1, 'J6', SALARY*1.15, 'J5', SALARY*1.2, SALARY*1.05) AS "인상된 급여"
 FROM EMPLOYEE
 ...
 */
 -- 전체 사원의 총 급여 합
 SELECT TO_CHAR(SUM(SALARY), 'FM999,999,999') --앞에 FM을 붙이면 공백제거됨
 FROM EMPLOYEE;
 ...
 /* 
       AVG(NUMBER)
       - 해당 컬럼값들의 평균값을 반환
       - 모든 그룹 함수는 NULL 값을 제외 -> AVG 함수를 사용할 때 NVL 함수와 사용하는 것을 권장
 */
 -- 전체 사원의 평균 급여 조회
 SELECT  TO_CHAR(ROUND(AVG(SALARY)), 'FM999,999,999') "평균 급여"   --ROUND 뒤에 점아래 없애기
 FROM EMPLOYEE;
 
 SELECT AVG(BONUS) FROM EMPLOYEE;
 SELECT AVG(NVL(BONUS, 0)) FROM EMPLOYEE; --권장!! (평균은 NULL값인 사람이 포함안되야되고 그게 나눌때도 사람수로 포함이 안되어야 하기떄문)
