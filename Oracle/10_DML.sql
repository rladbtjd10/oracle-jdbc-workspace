 /*
      ...
      : 데이터 조작 언어로 테이블 값을 삽입(INSERT) 하거나, 수정(UPDATE)하거나, 삭제(DELETE)하는 구문이다.
...
-- 표현법 1)
 ...
 -- 표현법 3)
 -- EMPLOYEE 테이블에서 직원의 사번, 이름, 부서명, 입사일 조회해서 EMP_01 테이블 INSERT 하시오.
...
/*
     ...
     - UPDATE시 서브쿼리를 사용해서 서브 쿼리를 수행한 결과값으로 컬럼 값을 변경할 수 있다.
*/
...
WHERE EMP_ID IN (SELECT EMP_ID
                            FROM EMP_SALARY E
                            JOIN DEPT_COPY D ON (D.DEPT_ID = E.DEPT_CODE)
...
-- DEPARTMENT 테이블에서 DEPT_ID가 D1인 부서 삭제 (여기도 다시!)
