...
/*
    뷰 컬럼에 별칭 부여
    - 서브쿼리의 SELECT절에 함수식이나 산술연산식 기술되어 있을 경우 반드시 별칭 지정!
*/
...
/*
    ...
    2. 뷰에 포함되지 않은 컬럼 중에 베이스가 되는 컬럼이 NOT NULL 제약 조건이 지정된 경우
...
-- INSERT
INSERT INTO VW_JOB VALUES('J8', '인턴'); --에러 발생! (SQL 오류: ORA-00913: 값의 수가 너무 많습니다)
INSERT INTO VW_JOB VALUES('J8');

SELECT*FROM VW_JOB;

-- UPDATE
UPDATE VW_JOB SET JOB_NAME = '인턴' WHERE JOB_CODE = 'J7'; -- 에러발생! (SQL 오류: ORA-00904: "JOB_NAME": 부적합한 식별자)
UPDATE VW_JOB SET JOB_CODE = 'J0' WHERE JOB_CODE = 'J8';

-- DELETE
DELETE FROM VW_JOB WHERE JOB_NAME = '사원'; --에러 발생! (SQL 오류: ORA-00904: "JOB_NAME": 부적합한 식별자)
...
-- INSERT
--NOT NULL 제약 조건이 지정된 경우에 해당함
INSERT INTO VW_JOB VALUES('인턴'); --에러 발생 (ORA-01400: NULL을 ("KH"."JOB"."JOB_CODE") 안에 삽입할 수 없습니다) 
...
INSERT INTO VW_EMP_SAL VALUES(400, '홍길동', 3000000, 36000000); --에러발생! (SQL 오류: ORA-00942: 테이블 또는 뷰가 존재하지 않습니다)
...
UPDATE VW_EMP_SAL
SET 연봉 = 80000000
WHERE EMP_ID = 200; --에러 (산술연산으로 정의된 컬럼은 데이터 변경 불가능)(SQL 오류: ORA-01733: 가상 열은 사용할 수 없습니다)
...
-- INSERT
INSERT INTO VW_JOINEMP VALUES(300, '조세오', '총무부'); --에러
...
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200; --> 서브쿼리에 기술한 조건에 부합되지 않기 떄문에 변경 불가 
